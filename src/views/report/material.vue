<template>
  <div class="app-container">
    <el-row :gutter="20" type="flex" class="table-header" justify="space-between">
      ...
      <el-col :span="4"><el-button type="primary"><a :href="`${baseUrl}/report/material/excel`">导出excel</a></el-button></el-col>
    </el-row>
    <el-table
    ...
    >
      ...
      <el-table-column label="材料/规格编号" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.uid + '/' + scope.row.fid }}</span>
        </template>
      </el-table-column>
      <el-table-column label="耗用材料" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.width || 0 }}</span>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
import { material } from '../../api/report'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      material(serialize(this.filter)).then(response => {
